...
if args.packaged:
    ...
    editor=subprocess.Popen([godot,'--editor','--path',str(ROOT/'godot')],stdout=editor_log,stderr=subprocess.STDOUT)
    time.sleep(5)
...
try:
    with log.open('w') as stream:
        ...
        while process.poll() is None:
            ...
            if elapsed>args.stress_seconds+170:
                process.terminate()
    ...
    data['source_sha256']=source_fingerprint()
    data['mode']='exported release template' if args.packaged else 'editor game run with editor UI open'
