...
def source_fingerprint():
    paths=[]
    for directory in ['src','cmake','godot/scripts','godot/scenes','godot/shaders','godot/data','godot/assets']:
        paths += [p for p in (ROOT/directory).rglob('*') if p.is_file() and not p.name.endswith('.import')]
