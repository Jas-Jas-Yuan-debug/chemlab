...
## 2026-09-10 修正：器材、浓度与反应时间

- **不同结构的玻璃器材**：烧杯有壶嘴、壁厚和容量刻度；试剂瓶有肩部、细颈、瓶塞和标签。按实际内腔积分计算液面高度和倾斜体积，修复杯底放射状闪烁。新手与专业视图均已区分。
- **0.1 / 1 mol/L 配液**：HCl、NaOH、KOH、NaCl、KCl 最高支持 1 mol/L；超过 0.01 mol/L 使用独立原始 `pitzer.dat` 计算活度。两个界面均可调整浓度，其他原料保留各自上限。
- **中和速率与混合**：水及上述强电解质逐时间步计算 H⁺/OH⁻ 反应和上、下两区有限交换，显示探头 pH、上部 pH 和 µmol/s 中和速率。图横轴使用实际模拟时间，等待求解不计入反应时间。
...
选中溶液卡片和容器后，可在“溶液浓度”输入 0.1 或 1 mol/L，再点“用所选溶液重新配液”。专业区在右侧向下滚动至配液区。最高浓度随原料切换；初始 0.001 mol/L 示例不代表统一固定浓度。
...
- **14 项水/预配水溶液**：H₂O、HCl、NaOH、NaCl、KOH、KCl、CaCl₂、NaHCO₃、Na₂CO₃、H₂SO₄、Na₂SO₄、MgCl₂、MgSO₄、BaCl₂。25°C、初始 1–250 mL；HCl/NaOH/NaCl/KOH/KCl 为 0.00001–1 mol/L，其他溶质最高 0.01 mol/L，水为 0。浓溶液混合仅限水及这五种强电解质，见 [支持矩阵](docs/SUPPORT_MATRIX.md)。
...
本次修正只运行相关定向检查：浓溶液/中和原生检查、动力学界面与保存流程、受几何影响的倾倒回归，以及反应时钟/即时读数检查，均通过。提交审查另发现空瓶倾倒误重置反应状态，修正后仅核对该边界，亦通过。未重复全套或性能测试。范围和证据见 [本次定向记录](artifacts/reaction-dynamics-validation.json)。

供后续完整验证使用的 `python3 scripts/verify.py` 现注册 10 项原生科学测试、Godot 导入和 13 个界面/保存流程（2 个无窗口、11 个真实渲染流程）；注册不表示本次重新执行了全套。燃烧测试包含 20 组独立 Cantera 平衡参考、有限燃料耗尽、能量与元素收支；空间场测试包括点燃/停料冷却、30/60 FPS 一致性及完整快照恢复。新手流程检查目录搜索、液体转移、有限固体质量、连接、模式切换和保存恢复。
