...
目前支持 **17/30 项原料**的限定实验，以及自由落体、弹簧、单摆、热交换、直流电路和薄透镜。支持器材操作、实际物质转移、测量/曲线、JSON 保存恢复和 CSV 导出。另 13 项显示“尚未支持”，不计入可操作数量，也不承诺任意混合。
...
顶部切换化学/气液固/沉淀/物理实验；原料按名称、化学式或类别搜索。可添加烧杯、量筒、滴管、试剂瓶。搅拌棒仅供观察，pH/温度计是模型读数，未开放的操作有说明。
...
- 六类物理模型有参数、解析核对、固定模拟步、暂停/重复、三维状态与曲线。热量单独记账。
- 三种指示剂使用有来源的 pH—颜色近似，忽略微量加入对体系的影响；已核对变色范围端点。
- 实际渲染流程覆盖按钮、选择/拖动/防重叠、倾倒/停止、相平衡、六类物理实验、保存恢复、CSV 和错误保留。`artifacts/verification.json` 是最近一次完整验证记录。

化学采用固定 25°C、充分混合后的平衡，不模拟反应速率、燃烧、有机合成、任意氧化还原、反应放热或空间浓度场。均一分样保持强度量，真正混合再求平衡。液体、玻璃与部分教学器材是明确的视觉近似，未声称经过实物光学标定。

[支持矩阵](docs/SUPPORT_MATRIX.md) · [科学边界](docs/VALIDATION_SCOPE.md) · [气液固](docs/BATCH_EQUILIBRIUM.md) · [扩展溶液](docs/AQUEOUS_EXTENSION.md) · [沉淀](docs/PRECIPITATION.md) · [物理模型](docs/PHYSICS_MODELS.md) · [指示剂](docs/INDICATORS.md) · [保存/CSV](docs/SESSION_FORMAT.md) · [视觉验证](docs/VISUAL_MODEL.md) · [交接](HANDOFF.md)
