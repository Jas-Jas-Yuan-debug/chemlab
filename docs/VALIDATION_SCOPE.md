...
Phase 0 已运行验证，Godot 三维交互和 9 项原料的界面配液、转移、读数流程已通过检查。当前 **9/30** 项在限定的水溶液形态可操作；其余条目及固体/气体形态不计为完成。通过命令行算例本身不能计为产品原料完成。

首批测试在 25°C、1 atm、稀水溶液、密闭且不交换 CO₂ 的条件下进行。充分混合后求平衡；不计算速率、混合时间、热释放或空间浓度场。pH 是氢离子活度的负常用对数。

独立 iphreeqc_probe 采用 mol/kg 水。产品核心 Chemistry::prepare 接受 mL 与 mol/L，固定溶质的量，迭代调整水质量使 SOLN_VOL 匹配目标体积；81 组执行检查了体积与元素数量。一次转移由完整 SOLUTION_RAW + MIX 重新平衡，保留反应生成水对体积和氢/氧的影响。

已验证范围为 25°C、初始 1–250 mL、1e-5–0.01 mol/L；水浓度为 0。IDs 2–6 为单价强酸碱/盐混合；IDs 7–9 只支持相同原料或水的组合，避免在尚无气体/固相模型时给出误导结果。浓度角点与中点作为代表性测试，未承诺区间外或全部组合。
